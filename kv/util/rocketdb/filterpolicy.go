...
// FilterPolicy is a factory type that allows the RocketDB database to create a
...
type FilterPolicy struct {
	Policy *C.rocketdb_filterpolicy_t
}
...
func NewBloomFilter(bitsPerKey int) *FilterPolicy {
	policy := C.rocketdb_filterpolicy_create_bloom(C.int(bitsPerKey))
	return &FilterPolicy{policy}
}

func (fp *FilterPolicy) Close() {
	C.rocketdb_filterpolicy_destroy(fp.Policy)
}
