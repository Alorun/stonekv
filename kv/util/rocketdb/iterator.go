...
// Iterator is a read-only iterator through a RocketDB database. It provides a
...
// safe to call.
//
// GetError will only return an error in the event of a RocketDB error. It will
...
type Iterator struct {
	Iter *C.rocketdb_iterator_t
}

// Valid returns false only when an Iterator has iterated past either the
// first or the last key in the database.
func (it *Iterator) Valid() bool {
	return ucharToBool(C.rocketdb_iter_valid(it.Iter))
}
...
func (it *Iterator) Key() []byte {
	var klen C.size_t
	kdata := C.rocketdb_iter_key(it.Iter, &klen)
	if kdata == nil {
		return nil
	}
...
func (it *Iterator) Value() []byte {
	var vlen C.size_t
	vdata := C.rocketdb_iter_value(it.Iter, &vlen)
	if vdata == nil {
		return nil
	}
...
func (it *Iterator) SeekToFirst() {
	C.rocketdb_iter_seek_to_first(it.Iter)
}
...
func (it *Iterator) SeekToLast() {
	C.rocketdb_iter_seek_to_last(it.Iter)
}
...
func (it *Iterator) Seek(key []byte) {
	C.rocketdb_iter_seek(it.Iter, (*C.char)(unsafe.Pointer(&key[0])), C.size_t(len(key)))
}

// GetError returns an IteratorError from RocketDB if it had one during
// iteration.
//
// This method is safe to call when Valid returns false.
func (it *Iterator) GetError() error {
	var errStr *C.char
	C.rocketdb_iter_get_error(it.Iter, &errStr)
	if errStr != nil {
...
func (it *Iterator) Close() {
	C.rocketdb_iter_destroy(it.Iter)
	it.Iter = nil
}
