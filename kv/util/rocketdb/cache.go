...
// implement their own *C.rocketdb_cache_t and create a Cache.
//
...
func NewLRUCache(capacity int) *Cache {
	return &Cache{C.rocketdb_cache_create_lru(C.size_t(capacity))}
}

// Close deallocates the underlying memory of the Cache object.
func (c *Cache) Close() {
	C.rocketdb_cache_destroy(c.Cache)
}
