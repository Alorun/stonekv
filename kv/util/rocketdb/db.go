...
// rocketdb_approximate_sizes is called from Go-land.
void rocketgo_approximate_sizes(
    rocketdb_t* db,
    int num_ranges,
    char** range_start_key, const size_t* range_start_key_len,
    char** range_limit_key, const size_t* range_limit_key_len,
    uint64_t* sizes) {
  rocketdb_approximate_sizes(db,
                            num_ranges,
...
// DB is a reusable handle to a RocketDB database on disk, created by Open.
//
...
type DB struct {
	...
	// then it's your code that is racey, not rocketdb.
	//
	// This indicates if the DB is closed or not. RocketDB provides it's own closed
	...
	// panic by checking this flag in functions that RocketDB would have done
	// it's own panic. This is not protected by a mutex because it's a best case
	// attempt to catch invalid usage. If access in racey and gets to RocketDB,
	// so be it, the user will see the sigtrap panic.
	// Because RocketDB has it's own mutex to detect the usage, we didn't want to
...
func Open(dbname string, o *Options) (*DB, error) {
	...
	db := C.rocketdb_open(o.Opt, ldbname, &errStr)
	if errStr != nil {
		gs := C.GoString(errStr)
		C.rocketdb_free(unsafe.Pointer(errStr))
...
func DestroyDatabase(dbname string, o *Options) error {
	...
	C.rocketdb_destroy_db(o.Opt, ldbname, &errStr)
	if errStr != nil {
		gs := C.GoString(errStr)
		C.rocketdb_free(unsafe.Pointer(errStr))
...
func RepairDatabase(dbname string, o *Options) error {
	...
	C.rocketdb_repair_db(o.Opt, ldbname, &errStr)
	if errStr != nil {
		gs := C.GoString(errStr)
		C.rocketdb_free(unsafe.Pointer(errStr))
		return DatabaseError(gs)
...
func (db *DB) Put(wo *WriteOptions, key, value []byte) error {
	...
	var errStr *C.char
	// rocketdb_put, _get, and _delete call memcpy() (by way of Memtable::Add)
...
func (db *DB) Get(ro *ReadOptions, key []byte) ([]byte, error) {
	...
	defer C.rocketdb_free(unsafe.Pointer(value))
...
func (db *DB) Write(wo *WriteOptions, w *WriteBatch) error {
	...
	var errStr *C.char
	C.rocketdb_write(db.Ldb, wo.Opt, w.wbatch, &errStr)
	if errStr != nil {
		gs := C.GoString(errStr)
		C.rocketdb_free(unsafe.Pointer(errStr))
		return DatabaseError(gs)
...
func (db *DB) NewIterator(ro *ReadOptions) *Iterator {
	...
	it := C.rocketdb_create_iterator(db.Ldb, ro.Opt)
...
//
// Examples of property names (as understood by the engine) include
// "rocketdb.stats", "rocketdb.sstables", and "rocketdb.num-files-at-level0".
func (db *DB) PropertyValue(propName string) string {
	...
	value := C.GoString(C.rocketdb_property_value(db.Ldb, cname))
...
func (db *DB) NewSnapshot() *Snapshot {
	...
	return &Snapshot{C.rocketdb_create_snapshot(db.Ldb)}
}
...
func (db *DB) ReleaseSnapshot(snap *Snapshot) {
	...
	C.rocketdb_release_snapshot(db.Ldb, snap.snap)
}
